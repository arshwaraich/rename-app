...
@Component({
  selector: 'app-filename-builder',
  templateUrl: './filename-builder.component.html',
  styleUrls: ['./filename-builder.component.css']
})
export class FilenameBuilderComponent implements OnInit {
...
// Chars to display in the original container in drag and drop
  ...
  swapChar(indexOld, indexNew) {
    ...
    for (let file of this.filesNew) {
      ...
      // check for a special case when index is 0
      if (indexOld === 0) { 
        string = file.name.substr(1, indexNew) + tempString +
        file.name.substr(indexNew + tempString.length);
      }
      else{
        string = file.name.substr(0, indexOld) + 
  ...
  drop(event: CdkDragDrop<string[]>) {
    if (event.previousContainer === event.container) {
      ...
      let tempPreviousData = event.previousContainer.data[event.previousIndex];
