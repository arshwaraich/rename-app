...
@Component({
  selector: 'app-increment-number',
  templateUrl: './increment-number.component.html',
  styleUrls: ['./increment-number.component.css']
})
export class IncrementNumberComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let i = 0;
    while (flag) {
      if (!sessionStorage.getItem('Seq' + i)) {
        flag = false;
      } else {
        i++;
      }
    }
    this.Strings.push('Seq' + i);
  }

  calculateSeq(startNum, maxDigits, incrementSize, index): number {
    let seq = startNum;
    for (let i = 0; i < index; i++) {
      seq += incrementSize;
      if (maxDigits) {
        if (Math.floor(seq / 10) > maxDigits) {
          seq = startNum;
        }
      }
    }
  ...
  updateNamesWithString(newString, indexOld, indexNew) {
    for (let j = 0; j < this.filesNew.length; j++) {
      const file = this.filesNew[j];
      let fullString = '';
      let newStringPart = '';
      let indexN = 0;
      ...
      for (let i = 0; i < indexNew; i++) {
        if (this.isItemInStorage(i)) {
          ...
          } else if (this.largeName[i].match(/Seq/)) {
            const seq = JSON.parse(sessionStorage.getItem(this.largeName[i]));
            indexN += this.calculateSeq(seq.startNum, seq.maxDigits, seq.incrementSize, j);
          } else {
      ...
      newStringPart = this.calculateSeq(this.startNum, this.maxDigits, this.incrementSize, j).toString();
